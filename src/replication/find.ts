...
export async function replicationFind(
	squirrel: Squirrel,
	id: string,
	data: VQueryT.Find,
): Promise<Data[]> {
...
export async function replicationFindOne(
	squirrel: Squirrel,
	id: string,
	query: VQueryT.FindOne,
): Promise<Data | null> {
	logger.debug("REPLICATION", "[V-SQR-11-01] replicationFindOne, id:", id);
	const data = await findGetData(
		squirrel,
		{
			...query,
			dbFindOpts: {
	...
	logger.debug(
		"REPLICATION",
		"[V-SQR-11-02] squirrelTimeData count:",
		squirrelTimeData.length,
	);

	if (!squirrelTimeData.length) {
		logger.debug(
			"REPLICATION",
			"[V-SQR-11-03] no timestamps, returning first result",
		);
		return data[0];
	}
	...
	logger.debug(
		"REPLICATION",
		"[V-SQR-11-04] returning latest result with timestamp",
	);

	return first;
}

async function findGetData(
	squirrel: Squirrel,
	query: VQueryT.Find,
	id: string,
) {
