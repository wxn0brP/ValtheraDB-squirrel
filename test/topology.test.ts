...
describe("TopologyManager", () => {
	test("1. addServer ignores duplicate server ids", () => {
		...
		topology.addServer({
			id: "a",
			host: "http://changed.local/",
		});

		expect(topology.servers.size).toBe(1);
		expect(topology.servers.get("a")).toEqual({
			id: "a",
			host: "http://a.local/",
		});
	});
	...
	test("2. getEpoch returns the epoch active for a timestamp", () => {
		...
		expect(topology.getEpoch(100)).toEqual({
			start: 100,
			serverIds: [
				"a",
			],
		});
		expect(topology.getEpoch(250)).toEqual({
			start: 200,
			serverIds: [
				"b",
			],
		});
		expect(topology.getEpoch(300)).toEqual({
			start: 300,
			serverIds: [
				"c",
			],
		});
		expect(topology.getEpoch(999)).toEqual({
			start: 300,
			serverIds: [
				"c",
			],
