...
export function registerDbOp(squirrel) {
    squirrel.app.post("/db/:op", async (req, res) => {
        const data = req.body.query || req.body.params?.[0];
        ...
        if (squirrel.config.replicationEnabled) {
            switch (req.params.op) {
                ...
                default:
                    return {
                        err: false,
                        result: await replicationOther(squirrel, req.params.op, id, data)
                    };
            }
        }
        if (!id) {
            ...
            if (squirrel.config.allowFullScan) {
                ...
                return fullScanReq(squirrel, data, req.params.op, res);
            }
